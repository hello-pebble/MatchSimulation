...
- 수신 방식은 4가지 (콘솔에서 선택):
  ...
  4. **WebSocket** — 연결 1개를 유지하며 서버가 즉시 push (요청 0회),
     전송도 같은 연결로 가능한 양방향 채널
...
### WebSocket ws://host/ws/chat?matchId={id}&token={JWT} — 실시간 양방향
- 브라우저 WebSocket API가 커스텀 헤더를 못 실으므로 JWT는 `token` 쿼리 파라미터로 전달
- 인증/권한 검증은 **핸드셰이크(연결 수립 전)**에서 수행 — 실패 시 연결 거부
  (토큰 없음/위조/정지 계정, 비참여자, 미성사 매칭, 없는 매칭)
- 수신: 새 메시지가 저장되면 `ChatMessageResponse` JSON 프레임을 즉시 push
  (REST로 전송된 메시지도 push — 수신 경로 교차 호환)
- 전송: `{"content":"토요일 오후 좋아요!"}` 프레임 — REST POST와 동일한 검증·권한.
  보낸 본인에게도 push(echo, `mine=true`)된다
- 오류: 검증 실패는 `{"error":"..."}` 프레임으로 회신하고 연결은 유지
- 실측: push 지연 ≈ 0.009초, 연결 후 폴링 요청 0회
...
6. **대화방 목록**으로 성사된 매칭의 matchId 확인 → **대화방 열기** → 메시지 입력
   ...
   **자동 수신**(Short Polling 3초 / Long Polling 서버 대기 / WebSocket 실시간)으로 수신
   — WebSocket 모드에서는 전송도 같은 연결로 이루어진다
...
| ![채팅 WebSocket](images/10_user_chat_ws.png) | 1:1 채팅 — WebSocket 실시간 수신 (연결 유지, 폴링 요청 0회) |
